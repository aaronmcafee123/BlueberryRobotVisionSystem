...
def parse_args():
    p = argparse.ArgumentParser(description="Train a YOLOv12 detector on a split dataset")
    p.add_argument("--dataset_dir", required=True, help="Root with images/{train,val,test} and labels/{train,val,test}")
    p.add_argument("--weights", default="yolov12s.pt", help="YOLOv12 weights to start from")
    ...
    p.add_argument("--lr0", type=float, default=None, help="Initial learning rate (leave unset for Ultralytics default)")
    p.add_argument("--lrf", type=float, default=None, help="Final LR fraction (leave unset for default)")
